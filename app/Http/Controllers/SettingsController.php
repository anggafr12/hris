    ...
    public function store(Request $request)
    {
        ...
        foreach ($uploads as $field => $fileName) {
            if ($request->hasFile($field)) {
                $request->validate([
                    $field => 'image|mimes:png|max:20480',
                ]);
        ...
        // Set default untuk checkbox dan toggle
        $defaults = [
        ...
        foreach ($defaults as $field) {
            $request[$field] = $request->has($field) ? $request[$field] : 'off';
        }

        // Penentuan warna tema
        $themeColor = $request->color_flag === 'false' ? $request->theme_color : $request->custom_color;

        // Simpan setting ke database
        $settings = Utility::settings();
        $post = $request->except(['_token', 'custom_color', 'color_flag', 'logo', 'logo_light', 'favicon']);
        $post['theme_color'] = $themeColor;
